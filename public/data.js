...
window.APP_DATA = (function () {
  ...
  const init = {
    ...
    steps: [
      ...
      { layer: "ctrl", title: "读 CAP 寄存器",
        ...
        file: "drivers/nvme/host/pci.c", func: "lo_hi_readq(dev->bar + NVME_REG_CAP)" },
      ...
      { layer: "ctrl", title: "配置 Admin 队列",
        ...
        file: "drivers/nvme/host/pci.c", func: "nvme_pci_configure_admin_queue" },
      ...
      { layer: "ns", title: "注册控制器",
        desc: "nvme_init_ctrl() 初始化控制器实例(cntlid/实例号)，创建字符设备 /dev/nvme0 与 /sys/class/nvme/nvme0。",
        file: "drivers/nvme/host/core.c", func: "nvme_init_ctrl" },
  ...
  const S_down = [
    ...
    { layer: "sched", title: "IO 调度器裁决",
      ...
      file: "block/mq-deadline.c", func: "dd_dispatch_request" },
    ...
    { layer: "nvme_core", title: "写入提交队列 SQ",
      desc: "nvme_submit_cmds() 把 SQE 写进 NVMe 提交队列(内存中的 SQ)，并推进 sq_tail。",
      file: "drivers/nvme/host/pci.c", func: "nvme_submit_cmds", structs: ["nvme_queue"] },
    ...
    { layer: "hw", title: "控制器取 SQ → 执行",
      ...
      file: "(SSD 固件)", func: "nvme_command → nvme_completion" },
    { layer: "hw", title: "写完成项 CQE",
      desc: "命令完成后，控制器把完成项 CQE 写入完成队列 CQ，翻转 phase bit，并触发 MSI-X 中断。",
      file: "drivers/nvme/host/pci.c", func: "nvme_completion", structs: ["nvme_completion"] },
  ...
  const read = {
    ...
    steps: [
      ...
      { layer: "fs", title: "未命中：预读 / 读入",
        ...
        file: "mm/readahead.c",
        func: { "4.19": "do_generic_file_read → readpage", "5.4": "read_pages → readpage", "5.15": "filemap_read_folio", "6.1": "filemap_read_folio → read_folio", "6.8": "read_pages → read_folio" },
